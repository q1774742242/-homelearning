...
function getAllByPage()
{
      $('#tb_departments').bootstrapTable({
            url:'/cp.do?method=allf&fid='+fid,        //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            dataType:'json',
            ...
            
            {
                ...
                align : 'center',
                formatter : function(value, row, index) {  
               	 var e = '<a href="#"  onclick="add(\''+ value + '\')">入库</a> ';  
...
function add(obj)
{
	 layer.open({
			...
			maxmin : true,
			area : [ '50%', '500px' ],
			content :  "/cp.do?method=myonef&fid="+obj,
...
function returnok(){
		$("#returnok").click(function(){
			window.location.href="/cp.do?method=index";
		})
