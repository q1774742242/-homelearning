...
function getAllByPage()
{
      $('#tb_departments').bootstrapTable({
            url:'/member.do?method=all',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            dataType:'json',
            ...
            },
            {
                ...
                align : 'center',
                formatter : function(value, row, index) {  
               	 var e = '<a href="#"  onclick="look(\''+ value + '\')">查看详情</a> ';     
                   return e;
                 }
            },
...
function edit(obj)
{
	 layer.open({
			...
			maxmin : true,
			area : [ '50%', '750px' ],
			content :  "/member.do?method=myone&rid="+obj,
...
function look(obj)
{
	layer.open({
		...
		maxmin : true,
		area : [ '50%', '750px' ],
		content :  "/member.do?method=look&rid="+obj,
		skin : 'layui-layer-grey',
		shift : 4, // 0-6的动画形式，-1不开启
		end : function() {
			window.location.reload();
...
function myadd()
{
	...
	$("#btn_add").click(function(){
		layer.open({
			...
			maxmin : true,
			area : [ '50%', '750px' ],
			content :  "/member.do?method=myadd",
			skin : 'layui-layer-grey',
			shift : 4, // 0-6的动画形式，-1不开启
			end : function() {
				 window.location.reload();
