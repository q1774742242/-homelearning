...
<script>
    ...
    function initBootstrapTable() {
        $('#rfidTable').bootstrapTable({
            url: '${base}/att/rfid/list',         //请求后台的URL（*）
            method: 'post',                      //请求方式（*）
            ...
            columns: [{
                ...
                align: "center",
                formatter: function (value, row, index) {
                    var btn = '<input class="switch" type="checkbox" name="useflg" id="useflg' + row.id + '"';
                    if (value) {
                        btn += 'checked';
                    }
                    btn += '/>'
    ...
    function operateFormatter(value, row, index) {//赋予的参数
        return [
            '<a href="javascript:void(0)" onclick="deleteOne(\'' + row.attRfidId + '\')" class="btn btn-danger"><span class="glyphicon glyphicon-remove" ></span>删除</a>  ',
        ].join('');
    }
    ...
    function initBootstrapSwitch() {
        //!* bootstrap开关控件，初始化 *!/
        $('input[name="useflg"]').bootstrapSwitch({
            onText: "是",      // 设置ON文本
            offText: "否",    // 设置OFF文本
        ...
        $('input[name="useflg"]').bootstrapSwitch("onSwitchChange", function (event, state) {
            ...
            updateUseFlg(id, flag);
        });
    }

    //修改使用状态
    function updateUseFlg(id, flag) {
        $.ajax({
            url: '${base}/sys/rfid/edit',
            type: 'post',
            dataType: "json",
            contentType: "application/json",
            data: JSON.stringify({'id': id, 'useflg': flag}),
            success: function (re) {
