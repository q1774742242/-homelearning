    ...
    <form class="form-horizontal" id="form">
        <div class="box-body">
            <div class="form-group">
                <label class="col-sm-2 control-label" for="end">出勤起始日</label>
                <div class="col-sm-6" >
                    <input type="text" id="tian" name="tian" class="form-control" value="${startDay}" placeholder="出勤起始日">
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-2 control-label" for="xiu">标准休息时间</label>
                <div class="col-sm-6" >
                    <input type="text" id="time" name="time" class="form-control" placeholder="标准休息时间">
                </div>
            </div>
        </div>
    ...
    function formValidator() {
        $("#form").bootstrapValidator({
            ...
            if (bv.isValid()) {
                ...
                var time = $("#time").val();
                var formdata={
                    "paraJson":JSON.stringify({startDay: tian,  timeOut: time}),
                };
                //发送ajax请求
                $.ajax({
                    url: "/att/modulePara/add",
                    type: 'POST',
                    dataType:"json",
