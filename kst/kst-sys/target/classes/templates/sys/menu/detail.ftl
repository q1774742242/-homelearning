    ...
    <form id="menuForm" class="form-horizontal" >
        ...
        <div class="box-body">
            <div class="form-group" >
                <label class="col-md-2 control-label"><@spring.message "sys.menu.name" /></label>
                <div class="col-md-7">
                    <input type="text" class="form-control" name="name" id="name" value="${menu.name}"/>
                </div>
            </div>
            <div class="form-group">
                <label class="col-md-2 control-label"><@spring.message "sys.menu.permission" /></label>
                <div class="col-md-4">
                    <input type="text" class="form-control" name="permission" id="permission" value="${menu.permission}"/>
                </div>
            </div>
            <div class="form-group">
                <label class="col-md-2 control-label"><@spring.message "sys.menu.icon" /></label>
                <div class="col-md-4">
                    ...
                    <button id="icon_add" class="btn btn-info" onclick="showIconModul()" type="button"><i class="fa fa-file-picture-o"></i> <@spring.message "common.button.selectIcon" /></button>
                </div>

            </div>
            <div class="form-group" >
                <label class="col-md-2 control-label"><@spring.message "sys.menu.type" /></label>
                <div class="col-md-2">
                    <select id="type" name="type" class="form-control selectpicker" title="<@spring.message "sys.message.choice" />" data-width="100px" >
            ...
            <div class="form-group" >
                <label class="col-md-2 control-label"><@spring.message "sys.menu.url" /></label>
                <div class="col-md-7">
                    <input type="text" class="form-control" name="href" id="href" value="${menu.href}" placeholder="<@spring.message "sys.menu.urlplace" />"/>
                </div>
            </div>
            <div class="form-group" >
                <label class="col-md-2 control-label"><@spring.message "sys.menu.sort" /></label>
                <div class="col-md-2">
            ...
            <div class="form-group" >
                <label class="col-md-2 control-label"><@spring.message "sys.user.isStartUsing" /></label>
                <div class="col-md-2">
                    <div class="switch">
        ...
        <#--<div class="box-footer pull-right">-->
            <button type="submit" id="btnConfirm" class="btn btn-primary" style="margin-right: 20px;display: none" ><@spring.message "common.button.save" /></button>
            <button type="button" class="btn btn-default" style="margin-right: 20px;display: none"><@spring.message "common.button.cancel" /></button>
        <#--</div>-->
    </form>
</div>
<!-- jQuery 3.3.1 -->
        ...
        $('#menuForm').bootstrapValidator({
            ...
            fields: {
                name: {
                    validators: {
                        notEmpty: {
                            message: '<@spring.message "sys.menu.name" /><@spring.message "sys.user.validate.notEmpty" />'
                        },
                        ...
                        remote: {//ajax验证。server result:{"valid",true or false} 向服务发送当前input name值，获得一个json数据。例表示正确：{"valid",true}
                            ...
                            message: '<@spring.message "sys.menu.name" /><@spring.message "sys.user.validate.alreadyExist" />',//提示消息
                ...
                permission: {
                    validators: {
                        ...
                        remote: {//ajax验证。server result:{"valid",true or false} 向服务发送当前input name值，获得一个json数据。例表示正确：{"valid",true}
                            ...
                            message: '<@spring.message "sys.menu.permission" /><@spring.message "sys.user.validate.alreadyExist" />',//提示消息
                            delay :  2000,//每输入一个字符，就发ajax请求，服务器压力还是太大，设置2秒发送一次ajax（默认输入一个字符，提交一次，服务器压力太大）
                            type: 'POST',//请求方式
