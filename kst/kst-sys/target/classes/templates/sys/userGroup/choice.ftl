...
<div class="box box-info" style="margin-bottom:0px;">
    ...
    <form class="form-horizontal" id="form">
        <div class="box-body">
            <div class="col-sm-4">
                <label for="organTree"><@spring.message "sys.userGroup.organText"/></label>
                <ul id="organTree" class="ztree">
                </ul>
                <label for="groupTree"><@spring.message "sys.userGroup.groupText"/></label>
        ...
        <!-- /.box-body -->
        <#--<input type="button" onclick="" id="btnConfirm" class="btn btn-primary" style="margin-right: 20px;display: none"><span class="glyphicon glyphicon-floppy-disk"></span>保存</input>-->
        <!-- /.box-footer -->
    </form>
</div>
...
<script type="text/javascript">
    ...
    function ztree() {
        ...
        $.ajax({
            ...
            type: 'post',
            success: function (data) {
                $.fn.zTree.init($("#organTree"), setting, data);
            }
        });
    }
    ...
    function loadUserTable() {
        $('#userTable').bootstrapTable({
            ...
            columns: [{
                ...
                field: 'nickName', title: '<@spring.message "sys.user.loginName" />'
            }, {
                field: 'tel', title: '<@spring.message "sys.user.tel" />',
            }]
        });
    }

    //上传选中的组织和用户组的用户
    function uploadGroupUser() {
        var organTree = $.fn.zTree.getZTreeObj("organTree");
        var organNodes = organTree.getSelectedNodes();
        var organIds = [];
        organNodes.forEach(function (r) {
            organIds.push(r.id);
        })
        ...
        retUsers.push.apply(retUsers, rows);
        ...
        $.ajax({
            ...
            success: function (data) {
                retUsers.push.apply(retUsers, data);
            }
        })
        return retUsers;
    }
