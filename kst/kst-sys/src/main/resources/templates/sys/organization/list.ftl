...
<script type="text/javascript">
    ...
    */
    function dt_remove(id) {
        layer.confirm('<@spring.message "sys.message.IsDeleteThis"/>', {icon: 3, title:'<@spring.message "sys.message.warning"/>'}, function (index) {
            $.ajax({
    ...
    */
    function addUserToOrgnization(){
        ...
        if(organId==null || organId=="" || organId==0){
            layer.alert("<@spring.message "sys.organization.message.choiceOrgan"/>", {icon: 7, title: '<@spring.message "sys.message.warning"/>'})
        }else{
            layer.open({
                type: 2,
        ...
        var setting = {
            ...
            callback: {
                ...
                onDrop:function (event, treeId, treeNodes, targetNode, moveType) {
                    ...
                    $.ajax({
                        url:'${base}/sys/organization/moveNode',
                        type:'post',
