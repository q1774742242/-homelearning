...
<div class="panel-body" style="padding-bottom:0px;">
	<div id="toolbar" class="btn-group">
		...
		<#--href='/activityView/create?name=activiti&key=123456'-->
		<#--<a type="button" class="btn  btn-success" onclick="draw()">-->
		<#--<i class="fa fa-trash" aria-hidden="true"></i>绘制流程-->
		<#--</a>-->
	</div>
	<table id="hiprocdefTable" ></table>
...
<script type="text/javascript">
	...
	function initBootstrapTable() {
		$('#hiprocdefTable').bootstrapTable({
			url: '${base}/hiprocdef/list',         //请求后台的URL（*）
			method: 'post',                      //请求方式（*）
	...
	function operateFormatter(value, row, index) {
		return [
			'<a href="#" onclick=draw() class="btn btn-primary">编辑</a>  ',
			'<a href="#" onclick="dt_publish(\'' + row.id + '\')" class="btn btn-primary">发布</a>  ',
			// '<a href="/activityView/publish?no="'+modelId+'" class="btn btn-info">发布</a> ',
			//dt_revokePublish
			'<a href="#" onclick="dt_revokePublish(\'' + row.id + '\')" class="btn btn-success">撤销</a>  ',
			//'<a href="/activityView/revokePublish?modelId=?" class="btn btn-success">撤销</a> ',
			//'<a href="/activityView/delete?modelId=?" class="btn btn-warning">删除</a> '
		].join('');
	}
	/**
	 * 打开新增框
	...
	function dt_insert() {
		...
		layer.open({
			...
			area: ['80%', '520px'],
			//area: ['80%', '80%'],
			content: '${base}/activityView/insert',
			btn: ['确定', '取消'],
	...
	function draw() {
		debugger;
		let row = $('#hiprocdefTable').bootstrapTable('getSelections');
		if (!row || row.length == 0) {
			toastr.error('请选择模型');
			return;
		}
		...
		layer.open({
			...
			//area: ['80%', '520px'],
			area: ['100%', '100%'],
			content: '/activityView/create'+'?name='+row[0].name+'&key='+row[0].key+'&editFlg='+true+'&modelId='+row[0].id,
			//btn: ['确定', '取消'],
			yes: function (index, layero) {
				debugger;
				// 获取弹出层中的form表单元素
	 ...
	 * @param id
	 */
	function dt_publish(id) {
		layer.confirm('确定要发布选中的记录？', {icon: 3, title:'提示'}, function (index) {
			$.ajax({
				url: '${base}/activityView/publish',
				type: "POST",
				data: {
					'id': id
				},
	 ...
	 * @param id
	 */
	function dt_revokePublish(id) {
		layer.confirm('确定要撤销发布选中的记录？', {icon: 3, title:'提示'}, function (index) {
			$.ajax({
				url: '${base}/activityView/revokePublish',
				type: "POST",
				data: {
					'id': id
				},
