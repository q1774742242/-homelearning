...
<script type="text/javascript">
	...
	function initBootstrapTable() {
		$('#exampleTable').bootstrapTable({
			url: '${base}/activityView/viewI',         //请求后台的URL（*）
			method: 'post',                      //请求方式（*）
	...
	function operateFormatter(value, row, index) {
		return [
			'<a href="#" onclick="draw()" class="btn btn-info">流程设计器</a>  ',
	...
	function draw() {
		debugger;
		let row = $('#exampleTable').bootstrapTable('getSelections');
		if (!row || row.length == 0) {
			toastr.error('请选择模型');
			return;
		}
