...
<div class="panel-body" style="padding-bottom:0px;">
    ...
    <div id="toolbar" class="btn-group">
        <button type="button" id="addBtn" onclick="addSupplier();" class="btn btn-primary" role="dialog">
            <span class="glyphicon glyphicon-plus" ></span>新增供应商
        </button>
...
<script>
    ...
    function dt_update(id) {
        ...
        layer.open({
            ...
            area: ['80%', '80%'],
            //area: '80%',
            content: '${base}/ams/supplier/edit/'+id,
            btn: ['确定', '取消'],
            yes: function (index, layero) {
                debugger;
                // 获取弹出层中的form表单元素
    ...
    function batchRemove() {
        var rows = $('#supplierTable').bootstrapTable('getSelections'); // 返回所有选择的行，当没有选择的记录时，返回一个空数组
        if (rows.length == 0) {
        ...
        layer.confirm("确认要删除选中的数据吗?", {icon: 3, title:'提示'}, function (index) {
            ...
            $.ajax({
                url: '${base}/ams/supplier/deleteSome',
                type: "POST",
                dataType:"json",
