...
<div class="box box-info" style="margin-bottom:0px;">
    ...
    <form class="form-horizontal" id="form">
        <input hidden="hidden" id="id" name="id" value="${location.id}"/>
        <input hidden="hidden" id="parentId" name="parentId" value="${location.parentId}"/>
        <input hidden="hidden" id="parentIds" name="parentIds" value="${location.parentIds}">
        <div class="box-body">
            <div class="form-group">
                <label class="col-sm-2 control-label" for="loginName">所属场所</label>
                <div class="col-sm-6" >
                    <input type="text" id="parent" class="form-control" readonly="readonly" value="${location.parentName}">
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-2 control-label" for="nickName">场所名</label>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-3">
                    <input type="text" id="sort" name="sort" class="form-control" value="${location.sort}">
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-2 control-label" for="email">等级</label>
                <div class="col-sm-3">
                    <input type="text" id="level" name="level" readonly="readonly" class="form-control" value="${location.level}">
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-2 control-label" for="phone">备注</label>
                <div class="col-sm-8">
                    <textarea id="remarks" name="remarks" class="form-control" rows="4">${location.remarks}</textarea>
                </div>
            </div>
        </div>
        <!-- /.box-body -->
    ...
    function formValidator() {
        ...
        $("#form").bootstrapValidator({
                ...
                name: {
                    validators: {
                        ...
                        remote: {//ajax验证。server result:{"valid",true or false} 向服务发送当前input name值，获得一个json数据。例表示正确：{"valid",true}
                            url: '${base}/ams/location/checkLocationName',//验证地址
                            message: '该场所已存在',//提示消息
            ...
            if (bv.isValid()) {
                ...
                var url = "";
                if(1 == mode){
                    url = "/ams/location/insert";
                }else{
                    url = "/ams/location/edit";
                }
