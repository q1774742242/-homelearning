	...
	$.jstree.core.prototype = {
      ...
      if (obj && $.inArray(obj.id, this._model.changed) === -1) {
		...
		_append_html_data : function (dom, data, cb) {
			 ...
			 */
			this.trigger('entity', { "nodes" : dpc, 'parent' : par });
			if(par !== $.jstree.root) {
			...
			if(this._data.core.selected.length !== s) {
				this.trigger('changed', { 'action' : 'entity', 'selected' : this._data.core.selected });
			}
			cb.call(this, true);
		},
		/**
		...
		_append_json_data : function (dom, data, cb, force_processing) {
			...
			var w = null,
				...
				rslt = function (rslt, worker) {
					...
					this.trigger('entity', { "nodes" : rslt.dpc, 'parent' : rslt.par });
		...
		create_node : function (par, node, pos, callback, is_loaded) {
			...
			this.trigger('entity', { "nodes" : dpc, "parent" : par.id });
		...
		copy_node : function (obj, par, pos, callback, is_loaded, skip_redraw, origin) {
			...
			dpc.push(node);
			dpc = dpc.concat(tmp.children_d);
			this.trigger('entity', { "nodes" : dpc, "parent" : new_par.id });
 ...
 */
	...
	$.jstree.plugins.checkbox = function (options, parent) {
		this.bind = function () {
			...
			if(!this.settings.checkbox.tie_selection) {
				this.element
					.on('entity.jstree', $.proxy(function (e, data) {
						var m = this._model.data,
							p = m[data.parent],
							dpc = data.nodes,
							i, j;
