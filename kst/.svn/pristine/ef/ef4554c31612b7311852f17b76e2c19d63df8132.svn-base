...
var addTabs = function (options) {
    ...
    if (options.urlType === "relative") {
        ...
        //options.href = basePath + options.href;
    ...
    if (findTabPanel(pageId) === null) {
        ...
        if (options.content) {
            ...
            var $iframe = $("<iframe></iframe>").attr("src", options.href).css("width", "100%").attr("frameborder", "no").attr("id", "iframe_" + pageId).addClass("tab_iframe").attr(pageIdField, pageId);
...
(function ($) {
    $.fn.sidebarMenu = function (options) {
        ...
        if (options.data) {
            ...
            $.getJSON(options.href, options.param, function (data) {
        ...
        function init($menu_ul, data, level) {
            $.each(data, function (i, item) {
                ...
                $title.addClass('menu-text').text(item.title);
                ...
                if (item.children && item.children.length > 0) {
                    ...
                    if (item.targetType != null && item.targetType === "blank") //代表打开新页面
                    {
                        ...
                        var href = 'addTabs({id:\'' + item.id + '\',title: \'' + item.title + '\',close: true,href: \'' + item.href + '\',urlType: \'' + item.urlType + '\'});';
                        $a.attr('onclick', href);
                    }
