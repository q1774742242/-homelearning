...
<div class="col-md-3" style="margin-top: 20px;">
    <button type="button" id="addRootBtn" onclick="addRootNode();" class="btn btn-group-sm btn-primary"
            role="dialog">
        <span class="glyphicon glyphicon-plus"></span>新增根目录
    </button>
...
<div class="panel-body col-md-9" style="padding-bottom:0px;">
    <div id="toolbar" class="btn-group" style="padding-bottom:5px;">
        <button type="button" id="upload" onclick="uploadFile()" class="btn btn-group-sm btn-primary"
                role="dialog">
            <span class="glyphicon glyphicon-remove"></span>上传文件
        </button>
        ...
        <button type="button" id="choiceType" onclick="choiceType()" class="btn btn-group-sm btn-info"
                role="dialog">
            <span class="glyphicon glyphicon-remove"></span>选择分类
        </button>
    </div>
...
<script type="text/javascript">
    ...
    */
    ...
    function operateFormatter(value, row, index) {//赋予的参数
        return [
            '<a href="#" onclick="editResource(\'' + row.id + '\')" class="btn btn-primary"><span class="glyphicon glyphicon-cog" ></span>修改</a>  ',
    ...
    function dt_remove(id) {
        layer.confirm('确定要删除这个文件？', {icon: 3, title: '提示'}, function (index) {
            $.ajax({
                ...
                if (typeId == null || typeId == "") {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            toastr.success('修改文件类型成功!');
                            $('#resourceTable').bootstrapTable('refresh');
                        },
                        error: function () {
                            toastr.error("修改文件类型失败")
                            $('#resourceTable').bootstrapTable('refresh');
                        }
                    });
                }
    ...
    function beforeRemove(treeId, treeNode) {
        var re = false;
        layer.confirm('确定要删除选中的目录？这将会使得其下的所有子目录以及目录中的文件都删除？', {icon: 3, title: '提示'}, function (index) {
            var data = {
                'id': treeNode.id,
    ...
    function uploadFile() {
        ...
        if (id == null || id == 0 || id == "") {
            layer.alert("请选择目录", {icon: 7, title: '警告'})
        } else {
            layer.open({
                type: 2,
                title: '上传文件',
