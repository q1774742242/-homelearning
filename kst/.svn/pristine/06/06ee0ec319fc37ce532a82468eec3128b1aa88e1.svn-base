...
<div class="panel-body" style="padding-bottom:0px;">
        ...
        <div class="panel-body">
            <form class="form-inline" id="searchForm">
                <div class="form-group" style="margin-right:20px">
                    ...
                    <input type="text" class="form-control _search" name="no" id="search_like_no" >
                </div>
                <div class="form-group" style="margin-right:20px">
                    <label for="s_name">资产带出状态</label>
                    <select name="takeout_userid" class="selectpicker _search" title="请选择"
                            data-width="150px">
                        <option value="">请选择</option>
                        <#list takeOutStatus as test>
    ...
    <div id="toolbar" class="btn-group">
        <button type="button" id="addBtn" onclick="addApply();" class="btn btn-primary" role="dialog">
            <span class="glyphicon glyphicon-plus" ></span>带出资产
        </button>
        <button type="button" id="delBtn" onclick="addReturn();"class="btn btn-warning">
            <span class="glyphicon glyphicon-remove" ></span>资产归还
        </button>
    </div>
...
<script>
    ...
    function initBootstrapTable() {
        ...
        $('#applyTable').bootstrapTable({
            url: '${base}/ams/takeout/list',         //请求后台的URL（*）ams:apply:list
            method: 'post',                      //请求方式（*）
            ...
            queryParams: function (params) {
                ...
                $("#searchForm ._search").each(function () {
                    if ("" != $(this).val()) {
                        searchParams[$(this).attr('name')] = $("#search_like_no").val();
    ...
    function dt_update(id) {
        ...
        layer.open({
            ...
            area: ['100%', '100%'],
            //area: '80%',
            content: '${base}/ams/takeout/edit/'+id,
            btn: ['确定', '取消'],
            yes: function (index, layero) {
                debugger;
                // 获取弹出层中的form表单元素
    ...
    function addReturn() {
        ...
        layer.confirm("确认要归还选中的数据吗?", {icon: 3, title: '提示'}, function (index) {
            ...
            $.ajax({
                url: '${base}/ams/takeout/returnAsset',
                type: "POST",
                dataType: "json",
    ...
    function batchRemove() {
        ...
        layer.confirm("确认要删除选中的数据吗?", {icon: 3, title:'提示'}, function (index) {
            ...
            $.ajax({
                url: '${base}/ams/takeout/deleteSome',
                type: "POST",
                dataType:"json",
                contentType:"application/json",
                data: JSON.stringify(rows),
